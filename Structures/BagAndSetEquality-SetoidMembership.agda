...
module Structures.BagAndSetEquality-SetoidMembership where
...
private
  ...
  module Ord {k ℓ c} (A : Setoid ℓ c) = Preorder (Related.InducedPreorder₂ k (_∈_ A))
...
-- c.f. Any.Membership-≡._∼[_]_
kind-eq : {ℓ c : Level} (A : Setoid ℓ c) → List (Setoid.Carrier A) → Related.Kind → List (Setoid.Carrier A) → Set (ℓ ⊔ c)
kind-eq A xs k ys = Ord._∼_ {k} A xs ys

infix -666 kind-eq
syntax kind-eq A xs k ys  =  xs ∼[ k ] ys ∶ A   -- ghost colon
...
++-cong : ∀ {k ℓ c} {A : Setoid ℓ c} {xs₁ xs₂ ys₁ ys₂ : List (Setoid.Carrier A)} →
          xs₁ ∼[ k ] xs₂ ∶ A → ys₁ ∼[ k ] ys₂ ∶ A →
          xs₁ ++ ys₁ ∼[ k ] xs₂ ++ ys₂ ∶ A
-- ++-cong {a} {xs₁ = xs₁} {xs₂} {ys₁} {ys₂} xs₁≈xs₂ ys₁≈ys₂ {x} = let open Related.EquationalReasoning in {!!}
++-cong {k} {ℓ} {c} {A} {xs₁ = xs₁} {xs₂} {ys₁} {ys₂} xs₁≈xs₂ ys₁≈ys₂ {x} =
  x ∈ₐ (xs₁ ++ ys₁)       ↔⟨ sym ++↔ ⟩
  (x ∈ₐ xs₁ ⊎ x ∈ₐ ys₁)  ∼⟨ xs₁≈xs₂ ⊎-cong ys₁≈ys₂ ⟩
  (x ∈ₐ xs₂ ⊎ x ∈ₐ ys₂)  ↔⟨ ++↔ ⟩
  x ∈ₐ xs₂ ++ ys₂       ∎
  where open Related.EquationalReasoning
        infix 4 _∈ₐ_
...
-- Musa Al-hassy: Note that, for my personal uses, I've removed the |Symmetric-kind| argument.
commutativeMonoid : {ℓ c : Level} → Setoid ℓ c → CommutativeMonoid ℓ (ℓ ⊔ c)
commutativeMonoid {ℓ} {c} A = record
  { Carrier             = List (Setoid.Carrier A)
  ; _≈_                 = λ xs ys → Ord._∼_ A xs ys
  ; _∙_                 = _++_
  ; ε                   = []
